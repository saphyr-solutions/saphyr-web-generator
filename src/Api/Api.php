...
class Api
{
    ...
    public function getModuleElements($moduleId)
    {
        try {
            ...
            if ($this->checkExpired($file, $lastModuleMod) || isset($_GET['test'])) {
                $result = $this->call('/api/v1/module/' . $moduleId . '/element', 'GET', ["byReference" => "1", "page" => "1", "perPage" => "0"]);
                if ($result) $this->cacheSave($file, $result);
            }
    ...
    public function getItems(array $request): array
    {
        ...
        if (!$module) throw new \Exception('Module ID/Name is required!');
        ...
        if (isset($request['order'])) {
            foreach ($request['order'] as $order) $query->addOrder($order[0], $order[1]);
        }
    ...
    public function getModuleElementField($moduleId, $fieldId)
    {
        ...
        if ($field) return array_shift($field);
        return false;
     ...
     * @return array|false|string|void
     */
    public function getModuleElementformConfig($moduleId)
    {
        try {
            $file = __FUNCTION__ . '_' . $moduleId . '.json';
            if ($this->checkExpired($file)) {
                $result = $this->call('/api/v1/module/' . $moduleId . '/element/formConfig');
                if ($result) $this->cacheSave($file, $result);
            }
            return ($this->getFromCache($file));
        } catch (\Exception $e) {
     ...
     * @return false|string
     */
    private function getFromCache($file): array
    {
        $path = $this->tempStorage . '__SC__' . $this->client . '_' . $file;
        if (!file_exists($path)) {
            return [];
    ...
    private function checkExpired($file, $lastItemMod = null)
    {
        ...
        if (!filesize($path)) return true;


        $lastMod = filemtime($path);
        if ($lastMod >= strtotime('-' . $this->ttl . ' second')) {
            if ($this->debug) {
                echo "Cache OK for " . $file . " : date (" . date('Y-m-d H:i:s', $lastMod) . ") within TTL (" . $this->ttl . " seconds)\n" . PHP_EOL;
            }
            return false;
        }
        ...
        if ($lastItemMod && $lastMod >= $lastItemMod) {
            if ($this->debug) {
                echo "Cache OK for " . $file . " : Module date (" . date('Y-m-d H:i:s', $lastItemMod) . ") older than file (" . date('Y-m-d H:i:s', $lastMod) . ")\n" . PHP_EOL;
            }
            return false;
        }
    ...
    private function call($action, $method = 'get', $params = null)
    {
        ...
        if ($result) {
            ...
            if (is_array($return) || is_object($return)) return $return;
            return $result;
        } else {
            return null;
        }
